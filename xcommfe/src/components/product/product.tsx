...
export default class Product extends React.Component<IProps, IState> {
    ...
    newProduct: IProduct ={
        id: 0, variantId:0, initial:'', name:'', description:'', price:0, stock:0, active: false, variant: this.newVariant
    }
    ...
    loadProducts = async () => {
        ...
        if (result.success) {
            const varResult = await VariantService.getAll();
            if (varResult.success) {
                this.setState({
                    variants: varResult.result
                });
            }
            const catResult = await CategoryService.getAll(pagination);
            if (catResult.success) {
    ...
    render() {
        const { categories, variants, products, product, showModal, command } = this.state;
        return (           
            <div>
                ...
                <tbody>
                    {
                        products?.map((o: IProduct) => {
                            return <tr key={o.id} className="border-b bg-gray-800 border-gray-700">                               
                                <td scope = "row" className="px-6 py-4">
                                    {o.variant.category?.initial}/
                                    {o.variant.initial}
                                </td>
                                ...
                                <td className="px-6 py-4">
                                    ...
                                    <input checked={o.active} id="checked-checkbox" type="checkbox" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                    </div>
                                </td>
                                ...
                                <div className="inline-flex" role="group" aria-label="Button group">
                                            ...
                                            <button className="h-8 px-4 text-blue-100 transition-colors duration-150 bg-blue-700 rounded-r-lg focus:shadow-outline hover:bg-blue-800" onClick={() => this.changeStatusCommand(o.id)}>Status</button>
                                        </div>
                                </td>
                            </tr>
                        })
